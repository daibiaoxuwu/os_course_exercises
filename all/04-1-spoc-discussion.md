...
1. 寄存器、高速缓存、内存、外存的访问特征？
   ...
   访问速度依次降低，频率依次降低，空间大小依次提升。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

   各个存储层次都处于满负荷繁忙状态，但是不会阻滞CPU的运转。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？

   遇到过。可以改变数据结构或者及时释放，循环利用内存空间。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

   覆盖是多个程序同时使用同一片内存，节省内存空间的技术。

   主要需要划分程序模块，处理好并行的共享关系。

2. 什么是交换技术？覆盖与交换有什么不同？

   交换是内存满时将不需要的页面换出去，将需要的换进来。

   覆盖是程序自身进行切块来重复利用资源。交换是操作系统为了节省资源，调度各个程序进程的过程。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

   在个人电脑上很少使用。可能用在资源需求较大的服务器端。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   时间局部性是刚访问过的地址空间有可能再次被访问。

   空间局部性是刚访问过的地址空间，它附近的空间有可能被访问。

   分支局部性是空间-时间路线只有几条固定的分支。
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   将某个程序进程的空间直接完整地放入物理内存中。

   虚拟存储占满内存后可能利用覆盖和交换，使得这块空间移到外存中。

   好处：实现简单，程序空间局部性更好。

   挑战：连续的大空间如何调度，如何对大片空间进行交换和覆盖操作，如何进行部分交换来节省内存空间。

2. 虚拟存储需要什么样的支持技术？

   操作系统的交换技术，如何选择应当交换的内存片。

   存储硬件只需要支持简单的地址线性转换即可。
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    ...
    当需要的页面没有被加载时，产生中断，操作系统暂停进程运行直到相应的页面被装入内存。

 2. 为了支持虚拟页式存储的实现，页表项有什么修改？

    页表需要增加一些标志位，包括驻留位，修改位，访问位，保护位。

 3. 页式存储和虚拟页式存储的区别是什么？

    页式存储只需要维护页表并在内存满时叫唤。虚拟页式存储还要通过别的机制，例如段机制来将先行地址转换为物理地址。

### 8.6 缺页异常

1. 缺页异常的处理流程？

   发生缺页时，查找外存中的页面。然后找到不常用的页面换掉，修改页表，然后回到引发异常的语句重新执行。
