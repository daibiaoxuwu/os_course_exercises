...
2. 不同特权级有什么区别？
   ...
   特权指令只能在0特权级访问，并且拥有更高的内存，寄存器等访问权限。


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

   CPL是程序的权限，DPL是内存段的权限，RPL是程序在请求内存段时的权限，常常低于或等于CPL。有时为了保证安全性会主动降低权限。
...
5. TSS和Task Register的作用是什么？
   ...
   TSS是任务状态段。用于任务切换，为操作系统j进程管理切换进程时保存上下文使用的信息。

   Task Register存放了当前任务的特权级等信息。
...
1. 一条指令执行时最多会出现多少次地址转换？
   ...
   2次。每个指令最多进行一次访存，在访存时需要进行段存储管理和页存储管理两次转换。

2. 描述X86-32的MMU地址转换过程；

   通过段寄存器转换为线性地址，再查找页表转换为物理地址。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

   CR0寄存器是控制寄存器，其中一个置位决定是否是保护模式，此步使CPU切换为保护模式；这时候MMU被使能，此时内存系统发生变化，PC下一次读取指令是将读取错误的地方。因此，需要ljmp语句跳转到$CODE_SEL处。这时MMU生效，PC指向正确的语句位置。

2. 分析页表的建立过程；

   页表在内核被装入内存时开始使用，映射8MB的内核空间。建立两个页表目录，每个1024项，每项大小为4字节，映射4K空间。然后，内核填充第0，1页给用户空间使用，填充第0x300，0x301页给内核空间使用，设置相应的物理空间和权限。然后，将两张页表的位置保存在GDTR和LDTR里。每个页表项有4字节，32位，其中20位用于保存物理地址空间，剩下12位作为标志位保存属性。
